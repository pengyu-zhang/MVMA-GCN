...
if __name__ == "__main__":
    ...
    sadj_pap, sadj_plp, sadj_pmp, fadj = load_graph(args.labelrate, config)
    ...
    model = SFGCN(nfeat=config.fdim,
                  ...
                  n_enc_1=config.n_enc_1,
                  n_enc_2=config.n_enc_2,
                  n_enc_3=config.n_enc_3,
                  n_dec_1=config.n_dec_1,
                  n_dec_2=config.n_dec_2,
                  n_dec_3=config.n_dec_3,
                  n_z=config.n_z,
    ...
    if cuda:
        ...
        sadj_pap = sadj_pap.cuda()
        sadj_plp = sadj_plp.cuda()
        sadj_pmp = sadj_pmp.cuda()
    ...
    def train(model, epochs):
        model.train()
        optimizer.zero_grad()
        output, att, emb1, com1, com2, emb2, emb3, emb4, com3, com4, emb, x_bar = model(features, sadj_pap, sadj_plp, sadj_pmp, fadj)
        loss_class = F.nll_loss(output[idx_train], labels[idx_train])
        re_loss = F.mse_loss(x_bar, features)
        loss_dep = (loss_dependence(emb1, com1, config.n) + loss_dependence(emb2, com2, config.n)
                    + loss_dependence(emb3, com3, config.n) + loss_dependence(emb4, com4, config.n)) / 4
        loss_com = common_loss(com1, com2, com3, com4)
        loss = loss_class + config.beta * loss_dep + config.theta * loss_com + re_loss
